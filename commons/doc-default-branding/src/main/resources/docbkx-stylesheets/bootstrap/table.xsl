...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="db ng exsl exslt oc stbl xtbl lxslt ptbl"
                version='1.0'>
  ...
  <xsl:template match="db:tgroup" name="tgroup">
    ...
    <table>
      ...
      <xsl:choose>
        ...
        <xsl:when test="../db:title">
          <xsl:attribute name="summary">
            ...
            <xsl:value-of select="string(../db:title)"/>
          </xsl:attribute>
        </xsl:when>
      ...
      <xsl:if test=".//db:footnote|../db:title//db:footnote">
        <tbody class="footnotes">
          <tr>
            <td colspan="{@cols}">
              <xsl:apply-templates select=".//db:footnote|../db:title//db:footnote" mode="table.footnote.mode"/>
            </td>
          </tr>
        </tbody>
      </xsl:if>
