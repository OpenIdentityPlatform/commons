...
public class RuntimeResultHandler {
    ...
    public boolean handleValidateRequestResult(AuthStatus authStatus, MessageInfo messageInfo, AuditTrail auditTrail,
        ...
        if (SUCCESS.equals(authStatus)) {
            ...
            JaspiRuntime.LOG.debug("Successfully validated request.");
            return true;
        } else if (SEND_SUCCESS.equals(authStatus)) {
            auditTrail.completeAuditAsSuccessful(getPrincipal(clientSubject));
            // Send HttpServletResponse to client and exit.
            JaspiRuntime.LOG.debug("Successfully validated request, with response message");
            return false;
        } else if (SEND_FAILURE.equals(authStatus)) {
            auditTrail.completeAuditAsFailure();
            // Send HttpServletResponse to client and exit.
            JaspiRuntime.LOG.debug("Failed to validate request, included response message.");
            response.setStatus(401);
            return false;
        } else if (SEND_CONTINUE.equals(authStatus)) {
            // Send HttpServletResponse to client and exit.
            JaspiRuntime.LOG.debug("Has not finished validating request. Requires more information from client.");
            return false;
        } else {
            auditTrail.completeAuditAsFailure();
            JaspiRuntime.LOG.error("Invalid AuthStatus, {}", AuthStatusUtils.asString(authStatus));
            throw new AuthException("Invalid AuthStatus from validateRequest: " + AuthStatusUtils.asString(authStatus));
        }
    }
    ...
    public void handleSecureResponseResult(final AuthStatus authStatus, final HttpServletResponse response)
        ...
        if (SEND_SUCCESS.equals(authStatus)) {
            // nothing to do here just carry on
            JaspiRuntime.LOG.debug("Successfully secured response.");
        } else if (SEND_FAILURE.equals(authStatus)) {
            // Send HttpServletResponse to client and exit.
            JaspiRuntime.LOG.debug("Failed to secured response, included response message");
            response.setStatus(500);
        } else if (SEND_CONTINUE.equals(authStatus)) {
            // Send HttpServletResponse to client and exit.
            JaspiRuntime.LOG.debug("Has not finished securing response. Requires more information from client.");
        } else {
            JaspiRuntime.LOG.error("Invalid AuthStatus, {}", AuthStatusUtils.asString(authStatus));
            throw new AuthException("Invalid AuthStatus from secureResponse: " + AuthStatusUtils.asString(authStatus));
        }
    }
}
